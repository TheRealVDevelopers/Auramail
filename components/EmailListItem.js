    ...
    React.createElement('li',
      ...
      React.createElement('button', { onClick: handleSelect, className: "w-full text-left p-3" },
        React.createElement('div', { className: "flex justify-between items-baseline" },
          React.createElement('p', { className: `text-sm font-semibold truncate ${email.read ? 'text-gray-600' : 'text-gray-900'}` }, email.folder === Folder.DRAFTS ? "Draft" : email.sender),
          React.createElement('p', { className: "text-xs text-gray-500 flex-shrink-0 ml-2" }, email.timestamp)
        ),
        React.createElement('p', { className: `text-sm truncate ${email.read ? 'text-gray-600 font-medium' : 'text-gray-800 font-semibold'}` }, email.subject || '(no subject)'),
        React.createElement('p', { className: "text-xs text-gray-500 truncate mt-1" }, email.body || '(no content)')
      )
    )
  );
};
