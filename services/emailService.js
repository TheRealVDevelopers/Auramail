 ...
 */
export const getUnreadCount = async (userId, folder) => {
    try {
...
export const getEmails = async (userId, folder) => {
    const emailsCol = getEmailsCollection(userId);
    // Fix: Use v8 `where` and `get` methods.
...
export const updateEmailFolder = async (userId, emailId, folder) => {
...
export const markEmailAsRead = async (userId, emailId) => {
    // Fix: Use v8 method chaining to get doc ref and `.update()` method
    const emailDoc = db.collection('users').doc(userId).collection('emails').doc(emailId);
    await emailDoc.update({ read: true });
};

export const seedEmailsForNewUser = async (userId) => {
    const emailsCol = getEmailsCollection(userId);
